...
export default class ConvertCMD extends LWCommand {
  constructor(print) {
    super(
      'convert', 'Converts from/to different units/bases. Currently supported: base, ascii, currency',
  ...
  static convertAscii(args) {
    ...
    // If the full table needs to be printed (ascii contains only 128 chars, extended ascii differs
    // from unicode)

    let output = '\t';
    let iter = 0;
    while (iter < 16) {
      output += `... ${parseInt(iter += 1, 10).toString(16).toUpperCase()}\t`;
    }
  ...
  static getAsciiDesc(char) {
    ...
    if (descriptions[char.toUpperCase()]) {
